...
===============================================================================
 ...
 Version     : v1.0
 date		 : 03/08/2016
 Description : Funciones del display tactil
===============================================================================
*/
...
void PIOINT1_IRQHandler(void)
{
	...
	if( flagsleep )
	{					// Si la Bandera de Sleep esta arriba es que el Display duerme y los despiesto.
...
void TOUCH_Standby(void)
{
	...
	GPIO_ClearInt(PORT1, 2);

	//X2P (+) GND.
//	LPC_PINCON->TOUCH_X2P = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT0,(1<<11),SALIDA);			//Seteo el Puerto como Salida!.
//	GPIOSetValue(PORT0,11,GND);				//Pongo el Pin X2P a GND.
	GPIO_ClearValue(PORT0, (1<<11));
	//X1M (-) DISABLE PULL UP & PULL DOWN - GPIO.
//	LPC_IOCON->TOUCH_X1M = 0xC1;			//Paso el Pin a Hi-z.
	GPIO_SetDir (PORT1,(1<<1),ENTRADA);			//Seteo el Pin como Entrada.

	//Y1M (-) ENABLE PULL UP / INT.
//	LPC_IOCON->TOUCH_Y1M = 0xD1;			//Paso el Pin a PULL UP.
	GPIO_SetDir (PORT1,(1<<2),ENTRADA);			//Seteo el Pin como Entrada.

	//Y2P (+) DISABLE PULL UP & PULL DOWN - GPIO.
//	LPC_IOCON->TOUCH_Y2P = 0xC1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,(1<<0),ENTRADA);			//Seteo el Pin Y2P como Entrada.
}
...
void TOUCH_GetX(void)
{
	...
	//X1M (-) GND.
//	LPC_PINCON->TOUCH_X1M = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,(1<<1),SALIDA);			//Seteo el Puerto como Salida!.
//	GPIOSetValue(PORT1,1,GND);				//Pongo el Pin X1M a GND.
	GPIO_ClearValue(PORT1, (1<<1));
	//X2P (+) VCC.
//	LPC_IOCON->TOUCH_X2P = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT0,(1<<11),SALIDA);			//Seteo el Puerto como Salida!.
//	GPIOSetValue(PORT0,11,VCC);				//Pongo el Pin X2P a VCC.
	GPIO_SetValue(PORT0, (1<<11));
...
void TOUCH_GetY(void)
{
	...
	//Y1M (-) GND.
//	LPC_PINCON->TOUCH_Y1M = 0x0D1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,(1<<2),SALIDA);			//Seteo el Puerto como Salida!.
//	GPIOSetValue(PORT1,2,GND);				//Pongo el Pin Y1N a GND.
	GPIO_ClearValue(PORT1, (1<<2));
	//Y2P (+) VCC.
//	LPC_IOCON->TOUCH_Y2P = 0x0D1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,(1<<0),SALIDA);			//Seteo el Puerto como Salida!.
//	GPIOSetValue(PORT1,0,VCC);				//Pongo el Pin Y2P a VCC.
	GPIO_SetValue(PORT1, (1<<0));
...
//	LPC_IOCON->TOUCH_X2P = 0xC9;			//Paso el Pin a Hi-z, Open Drain y GPIO.
	GPIO_SetDir (PORT0,(1<<11),ENTRADA);			//Seteo el Pin como Salida.
