...
void PIOINT1_IRQHandler(void){
	...
	GPIO_ClearInt(PORT1, 2);				// Limpio la Interrupcion.
...
//	GPIOIntDisable(PORT1, 2);
	...
	//X2P (+) GND.
//VER	LPC_IOCON->TOUCH_X2P = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT0,11,SALIDA);			//Seteo el Puerto como Salida!.
	GPIO_ClearValue(PORT0,11);				//Pongo el Pin X2P a GND.

	//X1M (-) DISABLE PULL UP & PULL DOWN - GPIO.
//VER	LPC_IOCON->TOUCH_X1M = 0xC1;			//Paso el Pin a Hi-z.
	GPIO_SetDir (PORT1,1,ENTRADA);			//Seteo el Pin como Entrada.

	//Y1M (-) ENABLE PULL UP / INT.
//VER	LPC_IOCON->TOUCH_Y1M = 0xD1;			//Paso el Pin a PULL UP.
	GPIO_SetDir (PORT1,2,ENTRADA);			//Seteo el Pin como Entrada.

	//Y2P (+) DISABLE PULL UP & PULL DOWN - GPIO.
//VER	LPC_IOCON->TOUCH_Y2P = 0xC1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,0,ENTRADA);			//Seteo el Pin Y2P como Entrada.
}
...
void TOUCH_GetX(void){
	...
	//X1M (-) GND.
//VER	LPC_IOCON->TOUCH_X1M = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,1,SALIDA);			//Seteo el Puerto como Salida!.
	GPIO_ClearValue(PORT1,1);				//Pongo el Pin X1M a GND.

	//X2P (+) VCC.
//VER	LPC_IOCON->TOUCH_X2P = 0xD1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT0,11,SALIDA);			//Seteo el Puerto como Salida!.
	GPIO_SetValue(PORT0,11);				//Pongo el Pin X2P a VCC.

	//Y1M (-) DISABLE PULL UP / ADC3.
//VER	LPC_IOCON->TOUCH_Y1M = 0x42;			//Paso el Pin a Hi-z y Open Drain.
	GPIO_SetDir (PORT1,2,ENTRADA);			//Seteo el Pin como Entrada.

	//Y2P (+) ENABLE PULL DOWN y GPIO.
//VER	LPC_IOCON->TOUCH_Y2P = 0xC9;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,0,ENTRADA);			//Seteo el Pin Y2P como Entrada.
...
void TOUCH_GetY(void){
	...
	//Y1M (-) GND.
//VER	LPC_IOCON->TOUCH_Y1M = 0x0D1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,2,SALIDA);			//Seteo el Puerto como Salida!.
	GPIO_ClearValue(PORT1,2);				//Pongo el Pin Y1N a GND.

	//Y2P (+) VCC.
//VER	LPC_IOCON->TOUCH_Y2P = 0x0D1;			//Paso el Pin a Puerto GPIO.
	GPIO_SetDir (PORT1,0,SALIDA);			//Seteo el Puerto como Salida!.
	GPIO_SetValue(PORT1,0);				//Pongo el Pin Y2P a VCC.

	//X1M (-) DISABLE PULL UP / ADC2.
//VER	LPC_IOCON->TOUCH_X1M = 0x42;			//Paso el Pin a Hi-z y Open Drain.
	GPIO_SetDir (PORT1,1,ENTRADA);			//Seteo el Pin como Entrada.

	//X2P (+) ENABLE PULL DOWN y GPIO.
//VER	LPC_IOCON->TOUCH_X2P = 0xC9;			//Paso el Pin a Hi-z, Open Drain y GPIO.
	GPIO_SetDir (PORT0,11,ENTRADA);			//Seteo el Pin como Salida.
