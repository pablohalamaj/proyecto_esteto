...
void SSP_Init( uint8_t portNum )
{
  ...
  /* Set DSS data to 8-bit, Frame format SPI, CPOL = 0, CPHA = 0, and SCR is 15 */
	LPC_SSP0->CR0 = 0x0707;//0x0707;

  /* SSPCPSR clock prescale register, master mode, minimum divisor is 0x02 */
	LPC_SSP0->CPSR = 0x02;//0xFE;//0x1E;//0x02
  ...
  else
  {
	...
	/* Set DSS (Data Size Select) to 16-Bit,
	 * Set FRF (Frame Format) to SPI,
	 * Set CPOL (Clock Out Polarity) = 0,
	 * Set CPHA (Clock Out Phase) = 0,
	 * Set SCR is 15 */
	LPC_SSP1->CR0 = 0x0700 | DSS_16_BIT_TRANSFER|CPOL_KEEP_LOW|CPHA_RISING_EDGE;
...
**
** parameters:			port #, buffer pointer (MODIFICADO PARA QUE ADMINTA 16 BITS), and the block length
** Returned value:		None
** 
*****************************************************************************/
